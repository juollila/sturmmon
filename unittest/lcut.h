...
/* modified by sturmmann 
#define LCUT_MAX_NAME_LEN 128
#define LCUT_MAX_STR_LEN 128 */
#define LCUT_MAX_NAME_LEN 24
#define LCUT_MAX_STR_LEN 80

/* modified by sturmmann */
#define LCUT_LOGO \
"**************************************** LCUT -- Lightweight C Unit Testing      By Tony Bai                            ****************************************"

#define SUCCESS_TIP_FMT "Case '%s': Passed\n"
//#define FAILURE_TIP_FMT "Case '%s': Failure occur in %s, %d line in file %s, %s\n"
#define FAILURE_TIP_FMT "Case '%s': %s\n"
...
#define LCUT_TEST_BEGIN(s, setup, teardown) \
    ...
    int   _cut_status; \
    int   _cut_result    = TEST_CASE_INVALID; \
...
void lcut_true(lcut_tc_t *tc, int condition,
               int lineno, const char *fcname, const char *fname);
*/
void lcut_int_equal(lcut_tc_t *tc, const int expected, const int actual);
void lcut_int_nequal(lcut_tc_t *tc, const int expected, const int actual);
void lcut_str_equal(lcut_tc_t *tc, const char *expected, const char *actual);
void lcut_str_nequal(lcut_tc_t *tc, const char *expected, const char *actual);
void lcut_assert(lcut_tc_t *tc, const char *msg, int condition);
void lcut_true(lcut_tc_t *tc, int condition);
...
void lcut_mock_obj_return(const char *symbol_name, void *value, const char *fcname, 
                          int lineno, const char *fname, int obj_type);
...
#define LCUT_ARG_RETURN(fcname, value) do { \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_ARG); \
    } while(0);

#define LCUT_RETV_RETURN(fcname, value) do { \
        lcut_mock_obj_return(#fcname, (void*)value, __FUNCTION__, __LINE__, __FILE__, MOCK_RETV); \
    } while(0);
