...
#define FILL_IN_FAILED_REASON(tc, reason_fmt, ...) do { \
...
void lcut_str_equal(lcut_tc_t *tc, const char *expected, const char *actual) {
    unsigned char nullStr[] = "NULL"; /* added by sturmmann */
    RETURN_WHEN_FAILED(tc);
...
/* modified by sturmmann */
    if(!expected) expected = nullStr;
    if(!actual) actual = nullStr;
    FILL_IN_FAILED_REASON(tc,
                          "expected<%s> : actual<%s>",
                           expected, actual);
}
            ...
            APR_RING_FOREACH(tc, &(ts->tc_head), lcut_tc_t, link) {
                if (tc != NULL) {
                    ...
                    if (tc->status == TEST_CASE_SUCCESS) {
                        ...
                        //       tc->fname, tc->reason);
                        printf(FAILURE_TIP_FMT, tc->desc, tc->reason);
...
/* modified by sturmmann */
//    return add_value(s, obj_type, value);
